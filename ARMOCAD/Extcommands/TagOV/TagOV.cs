    ...
    public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
    {
      ...
      try
      {
        using (Transaction t = new Transaction(doc, "Set TAGs"))
        {
          ...
          {
            var differentNumbers = g.GroupBy(i => i.LookupParameter("TAG").AsString().Split('-').Last().Remove(4));
          ...
          foreach (var g1 in handleElemsByT3T4)
          {
            ...
            foreach (var g2 in groupBySize)
            {
              ...
              foreach (var e in g2)
              {
                numCheck.Add(e.LookupParameter("TAG").AsString().Split('-').Last().Remove(4));
              }
            ...
            foreach (var g2 in groupByTag3Tag4)
            {
              ...
              if (handleTagT3T4.Count() > 0)
              {
                foreach (var i in handleTagT3T4)
                {
                  ...
                  t5Val = handleTag.Split('-').Last().Remove(4);
              ...
              foreach (var g3 in sortByFamilyAndSize)
              {
                ...
                {
                  t5Val = availableInts[n].ToString().PadLeft(4, '0');
                  n++;
                }
